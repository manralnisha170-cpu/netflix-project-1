...
def load_data():
    ...
    try:
        ...
        try:
            # If no saved results, load original
            st.sidebar.warning("Loading original dataset...")
            df = pd.read_csv('NetflixSimple.csv')
            return df
        except FileNotFoundError:
            # Last resort - try standard Netflix dataset name
            try:
                df = pd.read_csv('netflix_titles.csv')
                st.sidebar.success("✓ Data loaded successfully")
                return df
            except FileNotFoundError:
                st.sidebar.error("⚠️ Dataset not found! Please upload CSV file.")
                return None
...
def prepare_similarity_matrix(df):
    ...
    with st.spinner("🔄 Building recommendation engine... (this takes ~30 seconds)"):
        ...
        df['bag_of_content'] = (
            ...
            df['director'].astype(str) if 'director' in df.columns else '' + ' ' +
            df['cast'].astype(str) if 'cast' in df.columns else '' + ' ' +
            df['country'].astype(str) if 'country' in df.columns else ''
        )
...
def main():
    ...
    with st.sidebar:
        ...
        **No user ratings needed!** Pure content-based.
        ...
        if df is None:
            st.error("Please upload your Netflix dataset CSV file")
                                ...
                                if recommendations is not None:
                                    ...
                                    for idx, row in recommendations.iterrows():
                                        ...
                                        <div class='recommendation-card'>
                                            ...
                                            <p style='margin: 0.5rem 0;'>
                                                <strong>Genres:</strong> {str(row.get('listed_in', 'N/A'))[:80]}...
                                            </p>
                                            <p style='margin: 0.5rem 0; font-size: 0.9rem; color: #cccccc;'>
                                                {str(row.get('description', 'No description'))[:150]}...
                                            </p>
                                        </div>
                                        """, unsafe_allow_html=True)
